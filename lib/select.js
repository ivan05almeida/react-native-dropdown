...
class Select extends Component {
  ...
  _onPress() {
    ...
    const {
      optionListRef, children, onSelect, width, height, styleOption
    } = this.props;
    ...
    optionListRef()._show(children, this.pageX, this.pageY, width, height, styleOption, (item, value = item) => {
      if (item) {
        onSelect(value);
        this.setState({
          value: item
  ...
  render() {
    const {
      width, height, children, defaultValue, style, styleOption, styleText
    } = this.props;
    return (
